    ...
    <main className="kiosk-screen">
      ...
      <section className="kiosk-stage">
        <div className="kiosk-qr-panel">
          ...
          <div className="kiosk-countdown">{qr ? <RefreshCw size={44} /> : <CalendarDays size={44} />}<span>{qr ? <>QR diperbarui dalam <strong>{secondsLeft.toString().padStart(2, '0')}</strong> detik</> : data.qr_unavailable_reason || 'QR tampil sesuai waktu absensi.'}</span></div>
        </div>
