...
REVERB_ID="$(env_value backend/.env REVERB_APP_ID)"
REVERB_KEY="$(env_value backend/.env REVERB_APP_KEY)"
REVERB_SECRET="$(env_value backend/.env REVERB_APP_SECRET)"
...
set_env_value backend/.env REVERB_APP_ID "$REVERB_ID"
set_env_value backend/.env REVERB_APP_KEY "$REVERB_KEY"
set_env_value backend/.env REVERB_APP_SECRET "$REVERB_SECRET"
set_env_value frontend/.env VITE_REVERB_APP_KEY "$REVERB_KEY"

php backend/artisan storage:link >/dev/null 2>&1 || true

echo
echo "Dependency dan key aplikasi sudah siap."
echo "1. Isi koneksi database dan Redis di backend/.env."
